...
var PokemonSources = /** @class */ (function () {
    ...
    PokemonSources.prototype.intersectWith = function (other) {
        ...
        if (other.sourcesBefore || this.sourcesBefore) {
            ...
            if (other.sourcesBefore > this.sourcesBefore) {
                for (var _i = 0, _b = this.sources; _i < _b.length; _i++) {
                    var source = _b[_i];
                    var sourceGen = parseInt(source.charAt(0), 10);
                ...
                for (var _c = 0, _d = other.sources; _c < _d.length; _c++) {
                    var source = _d[_c];
                    var sourceGen = parseInt(source.charAt(0), 10);
...
var TeamValidator = /** @class */ (function () {
    function TeamValidator(format) {
        this.format = dex_1.Dex.getFormat(format);
        this.dex = dex_1.Dex.forFormat(this.format);
        this.gen = this.dex.gen;
        this.ruleTable = this.dex.getRuleTable(this.format);
    }
    TeamValidator.prototype.validateTeam = function (team, removeNicknames) {
        if (removeNicknames === void 0) { removeNicknames = false; }
    ...
    TeamValidator.prototype.baseValidateTeam = function (team, removeNicknames) {
        if (removeNicknames === void 0) { removeNicknames = false; }
        ...
        var _a = format.teamLength && format.teamLength.validate || [1, 6], minSize = _a[0], maxSize = _a[1];
        if (format.gameType === 'doubles' && minSize < 2)
            minSize = 2;
        if (['triples', 'rotation'].includes(format.gameType) && minSize < 3)
            minSize = 3;
        if (team.length < minSize)
            problems.push("You must bring at least " + minSize + " Pok\u00E9mon.");
        if (team.length > maxSize)
            return ["You may only bring up to " + maxSize + " Pok\u00E9mon."];
        ...
        for (var _i = 0, team_1 = team; _i < team_1.length; _i++) {
            ...
            if (!set)
                return ["You sent invalid team data. If you're not using a custom client, please report this as a bug."];
            var setProblems = (format.validateSet || this.validateSet).call(this, set, teamHas);
        ...
        for (var _b = 0, _c = ruleTable.complexTeamBans; _b < _c.length; _b++) {
            var _d = _c[_b], rule = _d[0], source = _d[1], limit = _d[2], bans = _d[3];
            var count = 0;
            for (var _e = 0, bans_1 = bans; _e < bans_1.length; _e++) {
                var ban = bans_1[_e];
                if (teamHas[ban] > 0) {
        ...
        for (var _f = 0, _g = ruleTable.keys(); _f < _g.length; _f++) {
            var rule = _g[_f];
            var subformat = dex.getFormat(rule);
    ...
    TeamValidator.prototype.validateSet = function (set, teamHas) {
        ...
        set.species = dex_1.Dex.getSpecies(set.species);
        set.name = dex.getName(set.name);
        var item = dex.getItem(dex_1.Dex.getString(set.item));
        set.item = item.name;
        var ability = dex.getAbility(dex_1.Dex.getString(set.ability));
        set.ability = ability.name;
        set.nature = dex.getNature(dex_1.Dex.getString(set.nature)).name;
        if (!Array.isArray(set.moves))
            set.moves = [];
        var maxLevel = format.maxLevel || 100;
        var maxForcedLevel = format.maxForcedLevel || maxLevel;
        var forcedLevel = null;
        if (!set.level) {
        ...
        var nameTemplate = dex.getTemplate(set.name);
        ...
        var setHas = {};
        var allowEVs = dex.currentMod !== 'letsgo';
        var capEVs = dex.gen > 2 && (ruleTable.has('obtainablemisc') || dex.gen === 6);
        if (!set.evs)
            set.evs = TeamValidator.fillStats(null, allowEVs && !capEVs ? 252 : 0);
        if (!set.ivs)
        ...
        var setSources = this.allSources(template);
        ...
        var learnsetTemplate = template;
        var tierTemplate = template;
        ...
        var problem = this.checkSpecies(set, template, tierTemplate, setHas);
        if (problem)
        ...
        if (ruleTable.isBanned('nonexistent')) {
            problems.push.apply(problems, this.validateStats(set, template, setSources));
        }
        var lsetProblem = null;
        for (var _a = 0, _b = set.moves; _a < _b.length; _a++) {
            var moveName = _b[_a];
            if (!moveName)
                continue;
            var move = dex.getMove(dex_1.Dex.getString(moveName));
            if (!move.exists)
            ...
            if (ruleTable.has('obtainablemoves')) {
                var checkLearnset = (ruleTable.checkLearnset && ruleTable.checkLearnset[0] || this.checkLearnset);
        ...
        var lsetProblems = this.reconcileLearnset(learnsetTemplate, setSources, lsetProblem, name);
        if (lsetProblems)
            problems.push.apply(problems, lsetProblems);
        if (!setSources.sourcesBefore && setSources.sources.length) {
            var legal = false;
            for (var _c = 0, _d = setSources.sources; _c < _d.length; _c++) {
                var source = _d[_c];
                if (this.validateSource(set, source, setSources, learnsetTemplate))
                    continue;
                legal = true;
                break;
            }
            if (!legal) {
                ...
                for (var _e = 0, _f = setSources.sources; _e < _f.length; _e++) {
                    var source = _f[_e];
                ...
                if (!nonEggSource) {
                    ...
                    var eventProblems = this.validateSource(set, nonEggSource, setSources, learnsetTemplate, " because it has a move only available");
                    if (eventProblems)
            ...
            var eventTemplate = !learnsetTemplate.eventPokemon && learnsetTemplate.baseSpecies !== learnsetTemplate.species ?
                dex.getTemplate(learnsetTemplate.baseSpecies) : learnsetTemplate;
            var eventPokemon = eventTemplate.eventPokemon;
            if (!eventPokemon)
                throw new Error("Event-only template " + template.species + " has no eventPokemon table");
            var legal = false;
            for (var _g = 0, eventPokemon_1 = eventPokemon; _g < eventPokemon_1.length; _g++) {
                var eventData = eventPokemon_1[_g];
                if (this.validateEvent(set, eventData, eventTemplate))
                    continue;
                legal = true;
                break;
            }
            ...
            if (!legal) {
                ...
                var eventInfo = eventPokemon[0];
                var minPastGen = (format.requirePlus ? 7 : format.requirePentagon ? 6 : 1);
                var eventNum = 1;
                for (var _h = 0, _j = eventPokemon.entries(); _h < _j.length; _h++) {
                    var _k = _j[_h], i = _k[0], eventData = _k[1];
                ...
                var eventName = eventPokemon.length > 1 ? " #" + eventNum : "";
                var eventProblems = this.validateEvent(set, eventInfo, eventTemplate, " to be", "from its event" + eventName);
        ...
        if (template.maleOnlyHidden && setSources.isHidden && setSources.sourcesBefore < 5 &&
        ...
        for (var _l = 0, _m = ruleTable.complexBans; _l < _m.length; _l++) {
            var _o = _m[_l], rule = _o[0], source = _o[1], limit = _o[2], bans = _o[3];
            var count = 0;
            for (var _p = 0, bans_2 = bans; _p < bans_2.length; _p++) {
                var ban = bans_2[_p];
                if (setHas[ban])
                    count++;
            }
        ...
        for (var _q = 0, ruleTable_2 = ruleTable; _q < ruleTable_2.length; _q++) {
            var rule = ruleTable_2[_q][0];
            if (rule.startsWith('!'))
                continue;
            var subformat = dex.getFormat(rule);
        ...
        if (!problems.length) {
            if (forcedLevel)
                set.level = forcedLevel;
            return null;
        }
        return problems;
    };
    TeamValidator.prototype.validateStats = function (set, template, setSources) {
        var ruleTable = this.ruleTable;
        var dex = this.dex;
        var allowEVs = dex.currentMod !== 'letsgo';
        var allowAVs = ruleTable.has('allowavs');
        var capEVs = dex.gen > 2 && (ruleTable.has('obtainablemisc') || dex.gen === 6);
        var canBottleCap = dex.gen >= 7 && (set.level === 100 || !ruleTable.has('obtainablemisc'));
        if (!set.evs)
            set.evs = TeamValidator.fillStats(null, allowEVs && !capEVs ? 252 : 0);
        if (!set.ivs)
            set.ivs = TeamValidator.fillStats(null, 31);
        ...
        if (set.hpType && maxedIVs && ruleTable.has('obtainablemisc')) {
            if (dex.gen <= 2) {
                var HPdvs = dex.getType(set.hpType).HPdvs;
        ...
        var cantBreedNorEvolve = (template.eggGroups[0] === 'Undiscovered' && !template.prevo && !template.nfe);
        var isLegendary = (cantBreedNorEvolve && ![
            'Unown', 'Pikachu',
        ].includes(template.baseSpecies)) || [
            'Cosmog', 'Cosmoem', 'Solgaleo', 'Lunala', 'Manaphy', 'Meltan', 'Melmetal',
        ].includes(template.baseSpecies);
        var diancieException = template.species === 'Diancie' && set.shiny;
        ...
        if (has3PerfectIVs) {
            ...
            if (perfectIVs < 3) {
                var reason = (this.format.requirePentagon ? " and this format requires gen " + dex.gen + " Pok\u00E9mon" : " in gen 6");
        ...
        if (dex.gen <= 2) {
            ...
            if (dex.gen > 1 && !template.gender) {
                ...
                var genderThreshold = template.genderRatio.F * 16;
                if (genderThreshold === 4)
                    genderThreshold = 5;
                if (genderThreshold === 8)
                    genderThreshold = 7;
     ...
     */
    TeamValidator.prototype.validateSource = function (set, source, setSources, template, because) {
        var eventData = null;
        ...
        if (source.charAt(1) === 'S') {
            ...
            if (eventTemplate.eventPokemon)
                eventData = eventTemplate.eventPokemon[parseInt(splitSource[0], 10)];
            if (!eventData) {
                throw new Error(eventTemplate.species + " from " + template.species + " doesn't have data for event " + source);
            }
        }
        else if (source.charAt(1) === 'V') {
            var isMew = template.speciesid === 'mew';
            var isCelebi = template.speciesid === 'celebi';
            eventData = {
                generation: 2,
    ...
    TeamValidator.prototype.findEggMoveFathers = function (source, template, setSources, getAll) {
        ...
        var dex = this.dex.gen === 1 ? dex_1.Dex.mod('gen2') : this.dex;
        ...
        if (eggGroups[0] === 'Undiscovered')
            eggGroups = dex.getTemplate(template.evos[0]).eggGroups;
        ...
        for (var fatherid in dex.data.Pokedex) {
            var father = dex.getTemplate(fatherid);
            ...
            if (template.speciesid === fatherid && !['dragonite', 'snorlax'].includes(fatherid))
                continue;
            ...
            if (father.evos.length) {
                var evolvedFather = dex.getTemplate(father.evos[0]);
     ...
     */
    TeamValidator.prototype.fatherCanLearn = function (template, moves, eggGen) {
        if (!template.learnset)
            return false;
        if (template.id === 'smeargle')
            return true;
        var eggMoveCount = 0;
        var noEggIncompatibility = template.eggGroups.includes('Field');
        for (var _i = 0, moves_1 = moves; _i < moves_1.length; _i++) {
            ...
            while (curTemplate) {
                if (curTemplate.learnset && curTemplate.learnset[move]) {
                    for (var _a = 0, _b = curTemplate.learnset[move]; _a < _b.length; _a++) {
                        var moveSource = _b[_a];
                        if (parseInt(moveSource.charAt(0)) > eggGen)
                            continue;
                        if (!'ESDV'.includes(moveSource.charAt(1)) || (moveSource.charAt(1) === 'E' && noEggIncompatibility)) {
                            canLearn = 2;
                            break;
                        }
                        else {
    ...
    TeamValidator.prototype.checkSpecies = function (set, template, tierTemplate, setHas) {
        var dex = this.dex;
        var ruleTable = this.ruleTable;
        ...
        var tier = tierTemplate.tier === '(PU)' ? 'ZU' : tierTemplate.tier;
        var tierTag = 'pokemontag:' + toID(tier);
        setHas[tierTag] = true;
        var doublesTier = tierTemplate.doublesTier === '(DUU)' ? 'DNU' : tierTemplate.doublesTier;
        var doublesTierTag = 'pokemontag:' + toID(doublesTier);
        setHas[doublesTierTag] = true;
        var banReason = ruleTable.check('pokemon:' + template.id);
        if (banReason) {
        ...
        if (tierTemplate.isNonstandard === 'Pokestar' && dex.gen === 5 ||
    ...
    TeamValidator.prototype.checkItem = function (set, item, setHas) {
        ...
        var banReason = ruleTable.check('item:' + item.id);
        if (banReason) {
     ...
     * If `because` is not passed, instead returns true if invalid.
     */
    TeamValidator.prototype.validateEvent = function (set, eventData, eventTemplate, because, from) {
        ...
        if (set.name && set.species !== set.name && template.baseSpecies !== set.name)
            name = set.name + " (" + set.species + ")";
        var fastReturn = !because;
        if (eventData.from)
            from = "from " + eventData.from;
        var etc = because + " " + from;
        var problems = [];
        ...
        if (eventData.ivs) {
            ...
            var canBottleCap = (dex.gen >= 7 && set.level === 100);
            if (!set.ivs)
        ...
        if (ruleTable.has('obtainableabilities')) {
            if (dex.gen <= 5 && eventData.abilities && eventData.abilities.length === 1 && !eventData.isHidden) {
                if (template.species === eventTemplate.species) {
                    ...
                    var requiredAbility = dex.getAbility(eventData.abilities[0]).name;
                    ...
                    // has evolved
                    var ability1 = dex.getAbility(eventTemplate.abilities['1']);
                    if (ability1.gen && eventData.generation >= ability1.gen) {
                        ...
                        var requiredAbilitySlot = (toID(eventData.abilities[0]) === ability1.id ? 1 : 0);
                        var requiredAbility = dex.getAbility(template.abilities[requiredAbilitySlot] || template.abilities['0']).name;
                        if (set.ability !== requiredAbility) {
                            var originalAbility = dex.getAbility(eventData.abilities[0]).name;
                            if (fastReturn)
                                return true;
            ...
            if (eventData.isHidden !== undefined && template.abilities['H']) {
                var isHidden = (set.ability === template.abilities['H']);
    ...
    TeamValidator.prototype.allSources = function (template) {
        var minPastGen = (this.format.requirePlus ? 7 :
            this.format.requirePentagon ? 6 :
                this.dex.gen >= 3 ? 3 : 1);
        if (template)
            minPastGen = Math.max(minPastGen, template.gen);
        var maxPastGen = this.ruleTable.has('allowtradeback') ? 2 : this.dex.gen;
        return new PokemonSources(maxPastGen, minPastGen);
    };
    TeamValidator.prototype.reconcileLearnset = function (species, setSources, problem, name) {
        if (name === void 0) { name = species.species; }
        var dex = this.dex;
        var problems = [];
        if (problem) {
            var problemString = name + "'s move " + problem.moveName;
            if (problem.type === 'incompatibleAbility') {
                ...
                var plural = (parseInt(problem.maxSketches, 10) === 1 ? '' : 's');
        ...
        if (setSources.size() && setSources.moveEvoCarryCount > 3) {
            ...
            setSources.sources = setSources.sources.filter(function (source) {
                return source.charAt(1) === 'E' && parseInt(source.charAt(0)) >= 6;
            });
            if (!setSources.size()) {
        ...
        if (setSources.isHidden) {
            setSources.sources = setSources.sources.filter(function (source) {
                return parseInt(source.charAt(0), 10) >= 5;
            });
        ...
        if (setSources.babyOnly && setSources.sources.length) {
            var baby_1 = dex.getTemplate(setSources.babyOnly);
            var babyEvo_1 = toID(baby_1.evos[0]);
            setSources.sources = setSources.sources.filter(function (source) {
                if (source.charAt(1) === 'S') {
                    var sourceSpeciesid = source.split(' ')[1];
                    if (sourceSpeciesid !== baby_1.id)
                        return false;
                }
                if (source.charAt(1) === 'E') {
            ...
            if (!setSources.sources.length && !setSources.sourcesBefore) {
                problems.push(name + "'s event/egg moves are from an evolution, and are incompatible with its moves from " + baby_1.species + ".");
            }
        }
        if (setSources.babyOnly && setSources.size()) {
            var baby_2 = dex.getTemplate(setSources.babyOnly);
            setSources.sources = setSources.sources.filter(function (source) {
                if (baby_2.gen > parseInt(source.charAt(0)))
                    return false;
            ...
            if (setSources.sourcesBefore < baby_2.gen)
                setSources.sourcesBefore = 0;
            if (!setSources.sources.length && !setSources.sourcesBefore) {
                problems.push(name + " has moves from before Gen " + baby_2.gen + ", which are incompatible with its moves from " + baby_2.species + ".");
            }
        }
        return problems.length ? problems : null;
    };
    TeamValidator.prototype.checkLearnset = function (move, species, setSources, set) {
        if (setSources === void 0) { setSources = this.allSources(species); }
        if (set === void 0) { set = {}; }
        var dex = this.dex;
        if (!setSources.size())
            throw new Error("Bad sources passed to checkLearnset");
        var moveid = toID(move);
        move = dex.getMove(moveid);
        var baseTemplate = dex.getTemplate(species);
        var template = baseTemplate;
        var format = this.format;
        var ruleTable = dex.getRuleTable(format);
        var alreadyChecked = {};
        var level = set.level || 100;
        var incompatibleAbility = false;
        var limit1 = true;
        var sketch = false;
        var blockedHM = false;
        ...
        while (template && template.species && !alreadyChecked[template.speciesid]) {
            ...
            var checkingPrevo = template.baseSpecies !== species.baseSpecies;
            ...
            if (template.learnset[moveid] || template.learnset['sketch']) {
                ...
                var lset = template.learnset[moveid];
                ...
                if (typeof lset === 'string')
                    lset = [lset];
                for (var _i = 0, lset_1 = lset; _i < lset_1.length; _i++) {
                    var learned = lset_1[_i];
                    ...
                    if (learnedGen <= moveSources.sourcesBefore)
                        continue;
                    if (learnedGen < 7 && setSources.isHidden &&
                    ...
                    if (!template.isNonstandard) {
                        ...
                        if (dex.gen >= 4 && learnedGen <= 3 &&
                            ['cut', 'fly', 'surf', 'strength', 'flash', 'rocksmash', 'waterfall', 'dive'].includes(moveid))
                            continue;
                        if (dex.gen >= 5 && learnedGen <= 4 &&
                            ['cut', 'fly', 'surf', 'strength', 'rocksmash', 'waterfall', 'rockclimb'].includes(moveid))
                            continue;
            ...
            if (ruleTable.has('mimicglitch') && template.gen < 5) {
                ...
                var getGlitch = false;
                for (var _a = 0, glitchMoves_1 = glitchMoves; _a < glitchMoves_1.length; _a++) {
                    var i = glitchMoves_1[_a];
            ...
            if (!moveSources.size()) {
                if ((template.evoType === 'levelMove' && template.evoMove !== move.name) ||
        ...
        if (!moveSources.size()) {
            if (minPastGen > 1 && sometimesPossible)
                return { type: 'pastgen', gen: minPastGen };
            if (incompatibleAbility)
                return { type: 'incompatibleAbility' };
            return { type: 'invalid' };
        }
    ...
    TeamValidator.prototype.learnsetParent = function (template) {
        if (template.species === 'Lycanroc-Dusk') {
            ...
            if (template.gen > Math.max(2, this.dex.gen))
                return null;
            return template;
        }
    ...
    TeamValidator.hasLegendaryIVs = function (template) {
