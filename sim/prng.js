...
var PRNG = /** @class */ (function () {
    ...
    Object.defineProperty(PRNG.prototype, "startingSeed", {
        /**
     ...
     */
    PRNG.prototype.next = function (from, to) {
     ...
     */
    PRNG.prototype.randomChance = function (numerator, denominator) {
    ...
    PRNG.prototype.sample = function (items) {
        ...
        var index = this.next(items.length);
     ...
     */
    PRNG.prototype.shuffle = function (items, start, end) {
        var _a;
        if (start === void 0) { start = 0; }
        if (end === void 0) { end = items.length; }
        while (start < end - 1) {
            var nextIndex = this.next(start, end);
            if (start !== nextIndex) {
                _a = [items[nextIndex], items[start]], items[start] = _a[0], items[nextIndex] = _a[1];
            }
            start++;
        }
    };
     ...
     */
    PRNG.prototype.nextFrame = function (initialSeed, framesToAdvance) {
        if (framesToAdvance === void 0) { framesToAdvance = 1; }
        // Use Slice so we don't actually alter the original seed.
        var seed = initialSeed.slice();
        for (var frame = 0; frame < framesToAdvance; ++frame) {
            ...
            for (var cN = seed.length - 1; cN >= 0; --cN) {
                ...
                for (var seedN = cN; seedN < seed.length; --aN, ++seedN) {
